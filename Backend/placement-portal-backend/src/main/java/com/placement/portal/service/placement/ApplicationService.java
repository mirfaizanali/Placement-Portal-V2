...
public class ApplicationService {
     ...
     * When transitioning to ACCEPTED, also flips the student's placement flag so
     * the Placement Officer dashboard reflects the new placement.
     */
    public ApplicationDto updateApplicationStatus(String id, ApplicationStatusUpdateRequest req) {
        ...
        if (newStatus == ApplicationStatus.ACCEPTED && previousStatus != ApplicationStatus.ACCEPTED) {
            markStudentPlaced(saved);
        }

        return entityMapper.toApplicationDto(saved);
    }

    /**
     * Sets {@code isPlaced=true} and {@code placedCompany} on the student associated
     * with the given accepted application. Allows overwriting a prior placement
     * (e.g., student accepted a better offer) and logs the transition.
     */
    private void markStudentPlaced(Application application) {
        StudentProfile student = application.getStudent();
        String companyName = resolveCompanyName(application);
        java.math.BigDecimal salary = resolvePackage(application);

        if (student.isPlaced()) {
            log.info("Student {} was already placed at '{}'; overwriting with '{}' (application {})",
                    student.getId(), student.getPlacedCompany(), companyName, application.getId());
        }
        ...
        if (salary != null) {
            student.setPlacementPackage(salary);
        }
        studentProfileRepository.save(student);
        log.info("Student {} marked PLACED at '{}' (package={}) via application {}",
                student.getId(), companyName, salary, application.getId());
    }

    private String resolveCompanyName(Application application) {
        if (application.getJob() != null && application.getJob().getEmployer() != null) {
            return application.getJob().getEmployer().getCompanyName();
        }
        if (application.getInternship() != null && application.getInternship().getEmployer() != null) {
            return application.getInternship().getEmployer().getCompanyName();
        }
        return null;
    }

    private java.math.BigDecimal resolvePackage(Application application) {
        if (application.getJob() != null) {
            java.math.BigDecimal max = application.getJob().getSalaryMax();
            if (max != null) return max;
            return application.getJob().getSalaryMin();
        }
        // Internships use stipend rather than package; leave null so analytics excludes them.
        return null;
    }
