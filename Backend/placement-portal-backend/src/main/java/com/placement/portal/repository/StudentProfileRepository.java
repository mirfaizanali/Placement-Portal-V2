...
public interface StudentProfileRepository extends JpaRepository<StudentProfile, String> {
    ...
    /**
     * Returns the highest placement package across all placed students.
     * Returns {@code null} if no students have been placed yet.
     */
    @Query("SELECT MAX(s.placementPackage) FROM StudentProfile s WHERE s.isPlaced = true")
    BigDecimal findHighestPackageOfPlacedStudents();
}
