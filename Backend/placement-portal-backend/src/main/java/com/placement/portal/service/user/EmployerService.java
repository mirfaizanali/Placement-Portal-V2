...
public class EmployerService {
    ...
    public EmployerProfileDto updateMyProfile(EmployerProfileUpdateRequest req) {
        String userId = securityUtils.getCurrentUserId();
        log.info("Updating employer profile for user {} with payload {}", userId, req);
        ...
        if (req.getCompanyWebsite() != null && !req.getCompanyWebsite().isBlank()) profile.setCompanyWebsite(req.getCompanyWebsite());
        if (req.getIndustry()       != null && !req.getIndustry().isBlank()) profile.setIndustry(req.getIndustry());
        if (req.getCompanySize()    != null && !req.getCompanySize().isBlank()) {
            try {
                profile.setCompanySize(CompanySize.valueOf(req.getCompanySize().toUpperCase()));
            } catch (IllegalArgumentException e) {
                throw new IllegalArgumentException(
                        "Invalid companySize '" + req.getCompanySize() +
                                "'. Allowed: " + java.util.Arrays.toString(CompanySize.values()));
            }
        }
        if (req.getHrContactName()  != null && !req.getHrContactName().isBlank()) profile.setHrContactName(req.getHrContactName());
        if (req.getHrContactPhone() != null && !req.getHrContactPhone().isBlank()) profile.setHrContactPhone(req.getHrContactPhone());
        if (req.getLocation()       != null && !req.getLocation().isBlank()) profile.setLocation(req.getLocation());
        if (req.getLogoUrl()        != null && !req.getLogoUrl().isBlank()) profile.setLogoUrl(req.getLogoUrl());
        if (req.getDescription()    != null && !req.getDescription().isBlank()) profile.setDescription(req.getDescription());

        try {
            EmployerProfile saved = employerProfileRepository.save(profile);
            return entityMapper.toEmployerProfileDto(saved);
        } catch (Exception e) {
            log.error("Failed to save employer profile for user {}", userId, e);
            throw e;
        }
    }
