...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleDataIntegrityViolationException(
        ...
        log.warn("DataIntegrityViolationException root cause: {}", rootMsg);

        String lower = rootMsg == null ? "" : rootMsg.toLowerCase();
        String message;
        HttpStatus status;
        if (lower.contains("cannot be null") || lower.contains("null value") || lower.contains("not-null")) {
            message = "Required field missing: " + rootMsg;
            status = HttpStatus.BAD_REQUEST;
        } else if (lower.contains("duplicate") || lower.contains("unique")) {
            message = "Resource already exists";
            status = HttpStatus.CONFLICT;
        } else {
            message = "Data integrity violation: " + rootMsg;
            status = HttpStatus.CONFLICT;
        }
